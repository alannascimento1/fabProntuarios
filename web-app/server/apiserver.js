...
app.get('/api/query/:pront_index', async function (req, res) {
 // to be filled in
    try {
        // load the network configuration
        ...
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        const result = await contract.evaluateTransaction('queryprontuario',req.params.pront_index);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
...
app.post('/api/addprontuario/', async function (req, res) {
 // to be filled in
    try {
        // load the network configuration
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        ...
        await contract.submitTransaction('createProntuario', req.body.prontId, req.body.cpf,req.body.nome,req.body.date, req.body.idade,req.body.informacao);
...
app.get('/api/history/:pront_index', async function (req, res) {
 // to be filled in
    try {
        // load the network configuration
        const ccpPath = path.resolve(__dirname, '..', '..', 'test-network', 'organizations', 'peerOrganizations', 'org1.example.com', 'connection-org1.json');
        ...
        // queryCar transaction - requires 1 argument, ex: ('queryCar', 'CAR4')
        // queryAllCars transaction - requires no arguments, ex: ('queryAllCars')
        const result = await contract.evaluateTransaction('getHistoryProntuarios',req.params.pront_index);
        console.log(`Transaction has been evaluated, result is: ${result.toString()}`);
